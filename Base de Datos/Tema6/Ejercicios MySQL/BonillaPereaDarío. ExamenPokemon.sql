...
-- Ejercicio 2 --
select *, concat("Esta es la generacion: ", id_generacion, 
". Y fue descubierta en el año: ", anyo_descubrimiento) as Mensaje
from generacion;

-- Ejercicio 3 --
select id_pokemon from entrenamiento
...
where left(nombre, 2) like '%a%' and left(nombre, 1) not like '%a%';

-- Ejercicio 6 --
select nombre, originario from pokemon
where length(nombre) > 5 and originario = 1;

-- Ejercicio 7 --
select p.id_pokemon, p.nombre 
from pokemon p
join entrenamiento e on p.id_pokemon = e.id_pokemon
where p.nombre not like '%Ch%' 
and (e.ataque > 80 or e.ataque_esp > 80);

-- Ejercicio 8 --
select id_pokemon from entrenamiento
where ataque_esp > (ataque + (ataque*20/100));
...
-- Ejercicio 10 --
select id_pokemon from entrenamiento
where (ataque_esp > 100 and defensa <= 90) or (defensa > 90 and ataque_esp <= 100);
...
-- Ejercicio 12 --
select nombre from entrenador
where right(nombre, 1) = '%J%' and left(nombre, length(nombre)-1) = '%o%';
...
-- Ejercicio 14 --
/*en caso de que se refiera el ejercicio a todos los pokemon de la última
generacion*/
select nombre from pokemon
where originario = 8;

/*en caso de que sea simplemente la ultima generación descubierta*/
select id_generacion, anyo_descubrimiento from generacion
order by anyo_descubrimiento desc
limit 1;

-- Ejercicio 15 --
select id_pokemon from entrenamiento
order by inicio_entrenamiento asc
limit 1;

-- Ejercicio 16 --
select id_pokemon from entrenamiento
where ataque_esp > (defensa + defensa_esp);

-- Ejercicio 17 --
select nombre, dinero from entrenador
where (dinero < 10000) or (dinero > 10000 and (nombre not like '%P%') and (nombre not like '%B%'));

-- Ejercicio 18 --
select id, nombre, precision_mov from movimiento
where precision_mov != 100;

-- Ejercicio 19 --
select distinct id_pokemon from entrenamiento;

-- Ejercicio 20 --
select nombre,
case 
when lleva_zapatillas = 1 
then concat("Enhorabuena ", nombre, ". Has ganado un total de ", 
dinero, " llevando puestas tus zapatillas.") 
else concat("Enhorabuena ", nombre, ". Has ganado un total de ", 
dinero, " sin llevar puestas tus zapatillas.") 
end as mensaje
from entrenador;